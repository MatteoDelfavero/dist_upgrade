...
ps aux | grep apt.systemd | grep -v grep | awk '{print $2}' | while read line; do kill -9 $line; done # kilojjuk a systemd-s processzeket amik foghatjak az apt-ot
sudo rm -f /var/lib/dpkg/lock* # es az ahhoz tartozo lockfajlt is

if [ -f $file ] && [ $(find $file -mmin -720 | wc -l) -eq 0 ]; then rm -f $file; fi # ha tobb mint fel napig nem sikerult frissiteni, ujrakezdjuk az egeszet

if [ -f $file ]; then
       
  if [ $(grep -E "\[FATAL\]|\[SUCCESS\]" $file | wc -l) -ne 0 ]; then # Ha minden lefutott, akkor nem megyunk tovabb
...
if [ -f $file ] && [ $(grep "IN PROGRESS" $file | wc -l) -ne 0 ]; then # ha fut folyamat, akkor nem megyunk tovabb
  echo -e "[INFO] Folyamat fut:\n$(cat $file)"
  exit 0
fi

sudo find /etc/ -type f -name "*unattended-upgrades*" | while read line; do if [ $(grep -i timeout $line | wc -l) -ne 0 ]; then echo $line; sed "s/1800/15/g" -i $line; fi ; done # alapesetben fel oraig varna rebootkor az unattended-upgrades servicere, ezt levesszuk 15mp-re
...
if [ -f $file ] && [ $(grep "$runtype OK" $file | wc -l) -ne 0 ] && [ $(grep "$runtype2 OK" $file | wc -l) -eq 0 ]; then
  ...
  if [ $(sudo cat /etc/os-release |grep VERSION_ID | grep 18 | wc -l) -ne 0 ]; then
	...
	yes | sudo apt purge chromium-browser-l10n -y # via Zsolt
	...
	sudo apt-get install -y -q # via Zsolt
  ...
  yes | sudo timeout 7200 apt upgrade -fy --allow-unauthenticated --with-new-pkgs </dev/null # ketto oraig futhat max
